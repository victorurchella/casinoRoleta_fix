...
if (bet == null) {
  $msg.html('Você tem que escolher uma cor')
} else {
var stawka = $('#stawka').val();
var saldo = $saldor.html();
if (+stawka < 100) {
  $msg.html('A aposta mínima deve ser de R$100,00')
} else if (+stawka > max_bet) {
  $msg.html('O valor máximo que você pode apostar é de R$50.000,00')
} else {
  if (+stawka > +saldo) {
    $msg.html('Você não tem dinheiro suficiente!')
  } else {
  foo.play();
 ...
 var  randomNumber = Math.floor(Math.random() * 36),
  ...
  setTimeout(function() {
    ...
    if (color == bet) {
      $thisResult = '<li style="color:#ffbf00;" class="previous-result color-'+ color +'"><span class="previous-number">'+ randomNumber +'</span><span class="previous-color">Você ganhou!</span></li>';
      $msg.html('<span style="color:#ffbf00;">Você ganhou!'+wygrana+'$');
      ...
      $thisResult = '<li class="previous-result color-'+ color +'"><span class="previous-number">'+ randomNumber +'</span><span class="previous-color">Você perdeu!</span></li>';
        losdio.play()
